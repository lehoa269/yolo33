...
for output in layerOutputs:
	...
	for detection in output:
		...
		if confidence > args["confidence"]:
			...
			# kích thước của hình ảnh, hãy nhớ rằng YOLO thực sự
			# trả về trung tâm (x, y) - các tọa độ của giới hạn
			# hộp theo sau là chiều rộng và chiều cao của hộp
			box = detection[0:4] * np.array([W, H, W, H])
			(centerX, centerY, width, height) = box.astype("int")

			# sử dụng trung tâm (x, y) để phối hợp đỉnh và
